...
if [ ! -f bq-metrics-extractor/cronjob/deploy-dev-direct.sh ]; then
...
TAG="${EXTRACTOR_VERSION}-${SHORT_SHA}-dev"
...
sed -e s/EXTRACTOR_VERSION/${TAG}/g bq-metrics-extractor/cronjob/extractor-dev.yaml | kubectl apply -f -
