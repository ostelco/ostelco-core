...
object Reporter {
    ...
    fun report(event: Event) {
        ...
        when (data) {
            ...
            is Subscription -> report(event, data)
                ...
                logger.warn(format("No handler found for Stripe event ${event.type}",
                        event))
            }
        }
    }
    ...
    private fun report(event: Event, balance: Balance) {
        when {
            event.type == "balance.available" -> logger.info(
    ...
    private fun report(event: Event, card: Card) {
        when {
            event.type == "customer.source.created" -> logger.info(
                    format("${email(card.customer)} added a new ${card.brand} ending in ${card.last4}",
                            event)
            )
            event.type == "customer.source.deleted" -> logger.info(
    ...
    private fun report(event: Event, customer: Customer) {
        when {
            ...
            event.type == "customer.updated" -> logger.info(
    ...
    private fun report(event: Event, source: Source) {
        when {
            ...
            event.type == "source.chargeable" -> logger.info(
    ...
    private fun report(event: Event, subscription: Subscription) {
        logger.warn(format("Unhandled Stripe event ${event.type} (cat: Subscription)",
                event))
    }
